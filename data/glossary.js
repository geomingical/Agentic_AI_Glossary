window.glossaryTerms = [
  ...
  },
  {
    ...
    searchTerms: ["chain-of-though", "chain of though"],
    note: "正式拼法是 Chain-of-Thought（最後有 t）。它是逐步推理的概念，不是與 SFT、RL 並列的訓練階段；推理範例可以用於訓練，也可以透過提示引導。產生步驟不保證答案正確，文字說明也不一定忠實反映模型內部運作。",
  ...
  },
  {
    ...
    searchTerms: ["Pre-trained", "Pretrained", "Pre trained"],
    note: "Pretraining／Pre-training 指預訓練這個過程；Pre-trained／Pretrained 是形容詞，表示模型已經過預訓練。不代表它已完成指令遵循或安全調整，後續仍可進行 Post-training。",
  ...
  },
  {
    ...
    searchTerms: ["Fine-tuned", "Finetuned", "Fine tuned"],
    note: "Fine-tuning 指微調過程；Fine-tuned 表示模型已經微調。微調會更新模型參數，不只是改提示詞或提供文件。SFT 是使用示範資料的監督式微調，並非所有微調都等於 SFT。",
    ...
    related: ["SFT", "Pretraining", "Instruction Tuning", "Base Model"],
  ...
  },
  {
    ...
    zh: "後訓練",
    category: "foundation",
    aliases: ["Post training", "Posttraining"],
    note: "Post-training 是預訓練之後的訓練階段，可包含 SFT、RL 或偏好最佳化等方法。常見流程是先 SFT 再進行 RL，但不是每個模型都採用相同順序或所有方法；RLHF 是其中利用人類回饋的路線。",
    ...
    related: ["SFT", "RL", "RLHF", "Instruction Tuning", "Alignment"],
  ...
  },
  {
    ...
    related: ["SFT", "Fine-tuning", "System Prompt", "Post-training"],
  ...
  },
  {
    ...
    zh: "監督式微調",
    category: "foundation",
    aliases: ["Supervised Fine-Tuning", "Supervised Fine Tuning"],
    summary: "用輸入與期望輸出的示範配對，進一步訓練已預訓練的模型。",
    analogy: "給清潔員一組工作單與標準處理示範，讓他學會遇到這類要求時該怎麼做。",
    definition: "Supervised fine-tuning adapts a pretrained model by optimizing it on labeled input-target examples, such as prompts paired with desired responses.",
    note: "例如輸入「請摘要這封信」，搭配一份理想摘要作為訓練目標。SFT 是一種微調方法；使用指令與回應配對的 SFT 是常見的 Instruction Tuning 做法。它主要向示範答案學習，不同於 RL 以獎勵評估行為。",
    related: ["Fine-tuning", "Instruction Tuning", "Post-training", "RL"],
    tags: ["training", "supervised", "examples"]
  },
  {
    term: "RL",
    zh: "強化學習",
    ...
    summary: "讓系統根據行動得到的獎勵訊號，學習如何選擇更好的行為。",
    analogy: "清潔員嘗試不同清潔方式，根據驗收得分調整策略，逐漸學會哪些做法更有效。",
    definition: "Reinforcement learning trains a policy to select actions that maximize expected cumulative reward from interaction with an environment.",
    note: "在語言模型後訓練中，可以對生成的答案評分，再更新模型。獎勵可來自人類偏好、獎勵模型或可驗證的結果，例如答案是否正確；RL 不一定需要人類逐筆回饋，也不等於 agent 在執行任務時自動持續訓練。",
    related: ["Post-training", "SFT", "RLHF", "Policy", "Environment"],
    tags: ["training", "reward", "policy"]
  },
  {
    term: "RLHF",
    zh: "人類回饋強化學習",
    ...
    note: "典型流程是由人類比較回答、用偏好資料訓練獎勵模型，再以 RL 調整語言模型。人類不必在每個訓練步驟直接評分；也不是所有使用人類偏好的方法都包含 RL，因此不能一概稱為 RLHF。",
    summary: "利用人類偏好回饋來調整模型，使輸出更有幫助或更符合期待。",
    analogy: "屋主比較兩種清潔成果並說哪個比較好，清潔公司據此調整訓練。",
    definition: "Reinforcement learning from human feedback optimizes a policy using reward signals derived from human feedback, commonly through a reward model trained on human preferences.",
    related: ["RL", "Post-training", "Alignment", "Evaluation"],
